...
export default function AdminSidebar({ role }: Props) {
  ...
  const docItems = allowed.filter((i) =>
    ["/upload-dokumen", "/raw-dokumen", "/glosarium", "/kategori"].includes(
      i.href
    )
  );
  ...
  const otherItems = allowed.filter(
    (i) =>
      !docItems.includes(i) && !orgItems.includes(i) && i.href !== "/dashboard"
  );
  ...
  return (
    <aside className="w-64 min-h-screen bg-foreground text-background flex flex-col">
      ...
      <div className="p-5 border-b border-background/10">
        ...
        <div className="text-xs uppercase tracking-wider text-background/40 mb-1">
          Workspace
        </div>
