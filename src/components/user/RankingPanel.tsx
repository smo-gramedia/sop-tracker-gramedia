...
export default function RankingPanel({
  ...
  return (
    <div className="bg-linear-to-br from-amber-50 via-orange-50 to-pink-50 rounded-2xl border border-amber-100 p-6 hover-lift">
      {/* Header */}
        ...
        {TIPE_LEADERBOARD.map((t) => (
          <button
            ...
            className={`flex-1 text-[11px] sm:text-xs px-2 py-1.5 rounded-lg transition-all ${
              tab === t
                ? "bg-white shadow-xs font-semibold text-foreground"
  ...
  return (
    <div
      className={`flex items-center gap-2.5 rounded-xl p-2 transition-all ${
        isCurrentUser
          ? "bg-linear-to-r from-blue-500/10 to-purple-500/10 border border-primary/30 shadow-xs"
          : isTop3
    ...
    >
      ...
      {/* Score */}
      <div className="shrink-0 text-right bg-white/80 rounded-lg px-2 py-1">
