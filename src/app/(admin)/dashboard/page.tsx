  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-8">
        ...
        <h1 className="font-display font-bold text-3xl mt-1">Selamat datang, {session?.user.name} 👋</h1>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        <StatCard icon={Users}      label="Total User Aktif"    value={totalUsers}   color="blue" />
        <StatCard icon={FileText}   label="Total Dokumen SOP"   value={totalDokumen} color="purple" />
        <StatCard icon={CheckCircle} label="Progress Selesai"   value={totalSelesai} color="green" />
        <StatCard icon={Clock}      label="Menunggu Verifikasi" value={menunggu}     color="amber" />
      </div>

      {/* Recent documents */}
      <div className="bg-background rounded-xl border">
        <div className="p-5 border-b">
          <h2 className="font-display font-semibold text-lg">Dokumen SOP Terbaru</h2>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/40">
              ...
              <th className="text-left px-5 py-3 font-medium text-muted-foreground">Tanggal</th>
          ...
          <tbody>
            {recentDokumen.map(doc => (
              <tr key={doc.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                ...
                <td className="px-5 py-3 text-muted-foreground">{doc.department?.nama ?? "—"}</td>
                <td className="px-5 py-3 text-muted-foreground">{formatTanggal(doc.createdAt)}</td>
...
function StatCard({ icon: Icon, label, value, color }: {
  ...
  return (
    <div className="bg-background rounded-xl border p-5">
      ...
      <div className="font-display font-bold text-3xl">{value.toLocaleString("id-ID")}</div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={cn("text-xs px-2 py-0.5 rounded-full border font-medium", map[status] ?? "bg-muted text-muted-foreground border-border")}>
