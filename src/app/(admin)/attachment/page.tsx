  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="font-display font-bold text-3xl mt-1">Attachment Sosialisasi SOP</h1>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/40">
              <th className="text-left px-5 py-3 font-medium text-muted-foreground">Nama User</th>
              <th className="text-left px-5 py-3 font-medium text-muted-foreground">SOP</th>
              <th className="text-left px-5 py-3 font-medium text-muted-foreground">Status</th>
              <th className="text-left px-5 py-3 font-medium text-muted-foreground">Upload ke-</th>
            ...
            {attachments.map(att => (
              <tr key={att.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                <td className="px-5 py-3">
                  ...
                  <div className="text-xs text-muted-foreground">{att.user.unit ?? "—"}</div>
                </td>
                <td className="px-5 py-3 text-sm">{att.sopDocument.judul}</td>
                <td className="px-5 py-3"><AttStatusBadge status={att.status} /></td>
                <td className="px-5 py-3 text-center text-muted-foreground">{att.uploadKe}</td>
                <td className="px-5 py-3 text-xs text-muted-foreground">{formatTanggal(att.uploadedAt)}</td>
                <td className="px-5 py-3">
...
function AttStatusBadge({ status }: { status: string }) {
  ...
  const labels: Record<string, string> = {
    menunggu: "Menunggu", disetujui: "Disetujui", ditolak: "Ditolak", pending: "Pending",
  };
  return (
