...
export default async function ProfilPage() {
  ...
  const dipelajari = progressList.filter(
      ...
      <div className="bg-background rounded-2xl border p-6 flex items-start gap-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-3 text-sm text-muted-foreground">
            ...
            {user?.joinedAt && (
              <span>Bergabung: {formatTanggal(user.joinedAt)}</span>
            )}
          </div>
        </div>
        <Link
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">
                    {SOP_KATEGORI_LABEL[p.sopDocument.kategori]} · Step{" "}
                    {p.stepCurrent}/6
                  </div>
                </div>
                <span
